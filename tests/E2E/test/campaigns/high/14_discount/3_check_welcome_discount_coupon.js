...
/*  scenario('Go to "Newsletter subscription" configuration page and activate the welcome discount coupon code', client => {
    ...
    test('should click on the "Configure" button', () => client.waitForExistAndClick(ModulePage.action_module_built_button));
    test('should set the "welcome voucher code"', () => client.waitAndSetValue(ModulePage.welcome_voucher_code, date_time));
  ...
  scenario('Configuration of SMTP mail server', client => {
    test('should go to "Advanced Parameters - Email" page', () => client.goToSubtabMenuPage(Menu.Configure.AdvancedParameters.email_submenu));
  ...
  scenario('Create a new account on the shopping cart, mark the box "Sign up for our newsletter" ', client => {
    ...
    test('should click on the "Sign up for our newsletter" button', () => client.waitAndSetValue(accountPage.save_newsletter_settings_button));
  ...
  scenario('Log into SMTP page and check the received mail ', client => {
    test('should set the shop language to "English"', () => client.snewWindow('http://0.0.0.0:1080'));
  }, 'common_client');
